...
插件和项目 `Content/JavaScript` 中的 `.js`、`.mjs`、`.cjs`、`.json` 由 `PuertsRuntimePlugin.Build.cs` 作为 NonUFS loose files staging。项目入口与插件包都必须保留原相对目录；`.map` 不进入 staging，Shipping 不依赖 source map。
